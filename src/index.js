...
import { TextControl, PanelBody, Button, DatePicker, RangeControl, ToggleControl, FontSizePicker } from '@wordpress/components';
...
registerBlockType( metadata.name, {
	...
	edit: function(props) {
		...
		const { attributes: { user_id, d1, d2, per_page, gridSize, gridGap, showHeader, headerFontSize }, setAttributes } = props;
        
		// Initialize the date values with today's date
		let now = new Date()
		if (!d1) {
			let dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
			setAttributes({ d1: dayStart.toISOString() });
		}
		if (!d2) {
			let tomorrowStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
			setAttributes({ d2: tomorrowStart.toISOString() });
		}
	...
	save: function(props) {
		const { attributes: { user_id, d1, d2, per_page, gridSize, gridGap, showHeader } } = props;
